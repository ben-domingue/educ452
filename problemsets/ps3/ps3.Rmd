...
## 4A
Can you build a DGM such that a teacher's decision to quit after the first year is governed by an entirely different process relative to that decision after subsequent years (and after the first year it can be governed by the exponential model we discussed in class)? Can you build it such that you can vary how strong this propensity to quit after the first year is?
...
## 4B
What kind of bias does this cause in estimates of lambda that come from fitting the naive exponential model? Ideally you can show how bias in lambda is varying as a function of the control parameter from (A)? 
