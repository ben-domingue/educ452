...
##What proportion of the distribution is more than 1.96 from the mean(=0)?
x<-rnorm(50000,mean=0,sd=1)
sum(abs(x)>1.96)/length(x) #what is this telling us?

##What does the sum of two normal variables look like?
N<-1000
m1<-3
sd1<-2
...
##Now let's look at a bivariate normal distribution
library(MASS)
